...
function OfferedCard({job}) {
  ...
  return (
     ...
     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
       ...
       <Link to={`/job-des/${job?.id}`}> <Button  variant='outline' color='green' >Accept</Button></Link>
       <Link to={`/job-des/${job?.id}`}> <Button bg="#FF4B4B">Reject</Button></Link>
